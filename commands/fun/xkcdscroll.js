...
const xkcdscroll=async(param, message)=>{
   ...
   if(param.length!=2||!isValidParam)
          return message.reply("the command argument provided for 'xkcdscroll' is invalid");

   const [ start,end ]=param.filter(e=>parseInt(e)).sort((a,b)=>a-b);

   if(start<1||end>2050)
          return message.reply("the bounds should be not exceed range 1-2050");
   if((end-start)>100)
          return message.reply("You cannot scroll across more than a 100 items");
...
const FetchXkcd=async(slideTab,fetchIndex)=>{
  ...
  if(!jet.slideTabs.has(userID)||fetchIndex>=data.length)
     return 
