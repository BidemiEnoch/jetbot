...
const mute=async(param,message)=>{
  ...
  const [ userMention,mutePeriod ]=param.split(/\s(.+)/);
   if(! mutePeriod)
      return message.reply("this command requires a user and time argument");
  const member=guild.members.get( getId(userMention) );
  if(!member) return message.reply(Errors.invalid_member);
  let time;
  try{  time=stringTimeToSeconds(mutePeriod);  }
  catch(e){ return message.reply(e.message); }
  
  if(time<60)
     return message.reply("the mute period should not be lower than 60seconds");
     
  const searchFn=r=>isMuteRole.test(r.name);
  
  const mutedRole=guild.roles.find(searchFn);
  if(!mutedRole) 
     return message.reply("this server has no 'muted' role");
  if(member.roles.has(mutedRole.id))
     return message.reply("this member has already been muted");
  const { id }=member;   
