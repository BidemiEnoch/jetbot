...
const unrole = async (param, message) => {
       ...
       if (!roleMention) return message.reply('this command requires a user and a role argument');
       ...
       const role = guild.roles.find((r) => searchRegex.test(r.name)) || guild.roles.get(getId(roleMention));
       if (!role) return message.reply(Errors.invalid_role);

       if (!member.roles.has(role.id)) return message.reply(`${member.displayName} does not have that role`);
       try {
