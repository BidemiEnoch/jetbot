...
const swapNicks = async (param, message) => {
       ...
       if (param.length < 2) return message.reply('this command requires the mention of two or more users');
       ...
       if (members.length === 2) displayNames = displayNames.reverse();
       else displayNames = shuffleArr(displayNames);
       try {
              for (const n in displayNames) await members[n].setNickname(displayNames[n]);
