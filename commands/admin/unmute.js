...
const unmute = async (param, message) => {
       ...
       if (!mutedRole) return channel.send('this member has no muted role');
       if (!jet.mutedUsers.has(member.id)) return channel.send('i did not instantiate the mute on this member,i cannot unmute them');
       try {
