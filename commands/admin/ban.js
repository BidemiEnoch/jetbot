...
const banMember=async(param,message)=>{
    ...
    try{
      ...
      channel.send(`${member.user.tag} has been banned from this server🔨,the messages they sent in the past ${days} days have been deleted`); }
    catch(e){ 
...
command.setDescription("bans a specified member from the server,the <number> argument specificies that the recent messages sent by the member in that amount of days should be deleted ");
