...
const help = async (param, message) => {
       ...
       for (const header in categories) {
              ...
              for (const [k, command] of category) {
                     ...
                     if (args) lines += `${k} (${args})${isOptional}\n`;
                     else lines += `${k}\n`;
              }
              embeds += `**${header}**\n\`\`\`${lines}\`\`\``;
       }
       ...
       try {
              ...
              if (channel.type !== 'dm') channel.send('you have been sent a dm📬');
       } catch (e) {
              if (channel.type !== 'dm') message.reply('i could not send you a dm');
...
const RunCommandHelp = (input, message) => {
       ...
       if (!commands.has(input)) return message.reply(Errors.invalid_command);
       const command = commands.get(input);

       if (command.access === 'owner' && author.id !== ownerID) return message.reply(Errors.no_access);
