...
const ev = (param, message) => {
       ...
       try {
              ...
              if (outputType === 'object' && !(output instanceof Map)) output = StringifyObject(output);
              else output = `${output}`;
              if (output.length < 200) channel.send(`\`\`\`code:${param}\noutput:${output}\ntype:${outputType}\n⌚️:${execTime}ms\`\`\``);
...
const UploadToHasteBin = async (channel, output, execTime, type) => {
       try {
              const { data } = await axios.post(`${Urls.hasteBin}/documents`, output);
              const { key } = data;
