...
const readfile = async (param, message) => {
       ...
       if (!/^\S+(\.js)?$/.test(param)) channel.send('invalid file path provided');
       param += param.endsWith('.js') ? '' : '.js';
       const m = await channel.send('searching for file...');
       try {
              const path = '/storage/emulated/0/@jet/' + param;
              const data = fs.readFileSync(path, 'utf8');
              if (data.length > 500) return PasteFileToHasteBin(param, m, data);
              const embed = jet.Embed.setTitle(`path(${param})`)
                     .addField('data', '```' + data + '```')
...
const PasteFileToHasteBin = async (param, message, file) => {
       try {
              ...
              const { data } = await axios.post(`${Urls.hasteBin}/documents`, file);
              const { key } = data;
              const embed = jet.Embed.setTitle(`path(${param})`)
