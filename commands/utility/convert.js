...
const convertFn = (param, message) => {
       ...
       if (args.length !== 2) return channel.send(Errors.long_input);
       ...
       try {
              ...
              message.reply(
                     'some of the conversion units provided where not recognised or the conversion types provided are incompatible,use `j:seeunits` to see all units'
              );
       }
};
