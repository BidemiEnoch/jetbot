...
const SeePermsEmbed=(data)=>{
  ...
  if(truepermsLength)
        trueperms=`\`\`\`${trueperms.join(", ")}\`\`\``;
  if(falsepermsLength)
        falseperms=`\`\`\`${falseperms.join(", ")}\`\`\``;
  const embed=jet.Embed
                              .setTitle(`${name}'s permissions`)
                              .setThumbnail(avatar)
...
command.setDescription("displays the acquired and unacquired permissions of specified member(s).displays your\'s if no member(s) are specified");
