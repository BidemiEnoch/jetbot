...
const roleinfo = (param, message) => {
       ...
       const role = guild.roles.get(getId(param)) || guild.roles.find((e) => pattern.test(e.name));
       if (!role) return message.reply(Errors.invalid_role);
       const { name, id, hexColor, calculatedPosition, hoist, members, createdAt } = role;
       const details = `position:${calculatedPosition}\nmembers:${members.size}`;
       const embed = jet.Embed.setTitle(`${name}(${id})`)
