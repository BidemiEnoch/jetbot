...
//  if(!commands.has(givenCommand)) 
  ...
  if(channel.type==="dm"&&command.isGuildOnly)
       return channel.send(Errors.no_dm);
  //checks if command is owner-only
  if(command.access==="owner"&&author.id!==ownerID)
       return channel.send(Errors.no_access);
  //checks if command is admin-only  
  if(command.access==="admin"&&!IsAdmin(guild,author.id))
       return channel.send(Errors.no_access);
  //checks permissions
  if(!jet.HasChannelPerms(message,command.perms)) return;
  //checks if arguments are being provided when not needed   
  if(!command.args&&value!=="")
       return channel.send(Errors.unrequired_args);
  //checks if arguments are not being provided when needed   
  if(command.args&&value===""&&!command.hasOptionalArgs)
        return channel.send(Errors.required_args(givenCommand));
