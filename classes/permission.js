...
const { Permissions,Client,Collection }=require("discord");
 ...
 constructor(){
  ...
  HasChannelPerms(message,permarr){
      ...
      for(const p in perms){
        if(!perms[p]&&permarr.includes(p)) 
              falsePerms.push(p.toLowerCase()); }
      if(falsePerms.includes("send_messages"))
          return false;
      if(!permarr.length) return true;
      if(falsePerms.length!==0)
          message.reply(Errors.permission(falsePerms));
