...
class Command{
	...
	setPerms(array){
		...
		if(! isValidArray)
		   throw new Error("invalid permissions were provided");
		this.perms=array;
		}
		
	setAccess(access){
		if(!/^(public|admin|owner)+$/.test(access))
		  throw new Error("an invalid access was provided");
		this.access=access; 
		}
	...
	setCategory(category){
		if(! categories.includes(category))
		  throw new Error("an invalid category was provided");
		return category;
	} 
